  ...
  <section>
    ...
    <el-col :span="24" class="warp-breadcrum">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item style="font-size: 20px">备件领用</el-breadcrumb-item>
      </el-breadcrumb>
    </el-col>
    ...
    <el-col :span="18" :class="this.Global.pageToolBarClass" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-select v-model="filters.form_num" placeholder="请选择领用单号" clearable filterable>
            <el-option
    ...
    <el-table :data="resultData" highlight-current-row  border v-loading="listLoading"
      ...
      <el-table-column prop="form_num" label="领用单号"
      ...
      <el-table-column prop="reciever" label="领用人"
                       min-width="180" show-overflow-tooltip header-align="center"  align="left">
        <template slot-scope="scope" style="color: #409EFF; cursor: pointer;">
          {{ scope.row.reciever }}
        </template>
      </el-table-column>
      <el-table-column prop="recive_date" label="领用日期"
                       min-width="180" show-overflow-tooltip  header-align="center"  align="left">
        <template slot-scope="scope" style="color: #409EFF; cursor: pointer;">
          {{ scope.row.recive_date }}
        </template>
      </el-table-column>
      ...
      <el-tabs v-model="activeTabName" type="border-card"
        ...
        <el-tab-pane label="基本信息" name="tabedit">
          ...
          <el-form :model="editForm" label-width="180px" :rules="editFormRules" ref="editForm" style="margin:-20px -20px -20px -20px; border:#DCDFE6 1px solid;">
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="领料单号" prop="form_num" :inline-message="true" style="margin-top: 10px;">
                  <el-input v-model="editForm.form_num"
                            placeholder="领料单号" maxlength="100" :disabled="editForm.id != 0"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <el-col :span="8" style="width: auto">
                <el-form-item label="领用人" prop="reciever" :inline-message="true" style="margin-top: 10px;">
                  <el-input v-model="editForm.reciever" placeholder="请输入领用人" maxlength="100" ></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="width: auto">
                <el-form-item label="领用日期" prop="recive_date" :inline-message="true" style="margin-top: 10px;">
                  <el-date-picker
                    v-model="editForm.recive_date"
                    type="datetime"
        ...
        <el-tab-pane label="领用明细" name="tablist">
          <span slot="label"><i class="el-icon-circle-plus"></i> 领用明细</span>
    ...
    <el-dialog :title="editTitle"  :visible.sync="viewFormVisible"  width="65%"
               :close-on-click-modal="false" v-dialogDrag>
      <div slot="title" style="font-size: 14px; font-weight: bold; color: #606266; " >
        <i class="fa fa-th-large" aria-hidden="true" style="margin-right: 10px;"></i>领用单详情
      </div>
          ...
          <el-form :model="editForm" label-width="180px" :rules="editFormRules" ref="editForm" style="margin:-20px -20px -20px -20px; border:#DCDFE6 1px solid;">
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="领料单号" prop="form_num" :inline-message="true" style="margin-top: 10px;">
                  <el-input v-model="editForm.form_num"
                            placeholder="领料单号" maxlength="100" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-form-item label="描述" prop="desc" :inline-message="true" style="margin-top: 10px;">
            ...
            <el-row>
              <el-col :span="8" style="width: auto">
                <el-form-item label="领用人" prop="reciever" :inline-message="true" style="margin-top: 10px;">
                  <el-input v-model="editForm.reciever"
                            placeholder="请输入领用人" maxlength="100" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="width: auto">
                <el-form-item label="领用日期" prop="recive_date" :inline-message="true" style="margin-top: 10px;">
  ...
  export default {
    ...
    methods: {
      ...
      f_EditGetDetail:function(pkid){
        ...
        let _this=this;
        this.$httpGet('eqpapi/V_EQP035M',pars).then((res)=> {
          if(res['msg'] =='success') {
      ...
      f_EditSubmit:function () {
        ...
        this.$refs.editForm.validate(() => {
          if(this.Global.isNullOrEmpty(_this.editForm.form_num)) {
            ...
            _this.$httpGet('eqpapi/S_EQP035M', pars).then((res) => {
              if (res['msg'] == 'success') {
      ...
      f_Delete: function (index, row, op_flag = 'M') {
        ...
        _this.$httpGet('eqpapi/D_EQP035M',pars).then((res)=> {
          _this.listLoading = false;
        if(res['msg']=='success') {
          _this.listLoading = false;
          _this.$message({
      ...
      f_DataSelecPatrsSearch:function(){
        ...
        let _this = this;
        this.$httpGet('eqpapi/Q_EQP035Find',pars).then((res)=> {
          if(res['msg'] == 'success') {
      ...
      f_ConfigLoad:function(){
        ...
        //注意POST和GET的URL，后面要加'/'
        this.$httpGet('eqpapi/Q_EQP035M',{}).then((res)=> {
          if(res['msg'] == 'success') {
      ...
      f_Search: function () {
        ...
        let params = {
          ...
          recive_date:this.filters.recive_date
        };

        _this.listLoading = true;

        _this.$httpGet('eqpapi/Qry035MStd', params).then((res) => {
          if (res['msg']== 'success') {
          _this.resultData = res['list'];
          _this.listLoading = false;
        } else {
