    ...
    <div>
      <el-col :span="24" class="warp-breadcrum">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item style="font-size: 20px">备件基本信息</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>

      <!--主页面搜索-->
      <el-col :span="17" :class="this.Global.pageToolBarClass" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters">
          <el-form-item>
            <el-input v-model="filters.PATRS_NUM" placeholder="备件编号"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-col :span="7"  :class="this.Global.pageToolBarClass" style="padding-bottom: 0px; text-align: right;">
        <el-form :inline="true">
          <el-form-item>
      ...
      <el-col :span="24" style="padding-bottom: 0px;">
        <el-table
          ...
          <el-table-column label="序号" align="center" sortable width="100px">
            <template slot-scope="scope">{{ scope.row.ID }}</template>
          </el-table-column>
          <el-table-column prop="PATRS_NUM" label="备件编号" align="center" sortable>
            <template slot-scope="scope">
              <el-input v-model="scope.row.PATRS_NUM"
                        :disabled="!(scope.row.ID == null || scope.row.ID == '' || scope.row.ID == 'undefined')">
              </el-input>
            </template>
          </el-table-column>
          <el-table-column prop="PATRS_NAME" label="名称" align="center" sortable>
            <template slot-scope="scope">
              <el-input v-model="scope.row.PATRS_NAME"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="PATRS_FMT" label="规格" align="center" sortable>
            <template slot-scope="scope">
              <el-input v-model="scope.row.PATRS_FMT"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="PATRS_CODE" label="备件分类" align="center" sortable>
            <template slot-scope="scope">
              <el-autocomplete
                class="inline-input"
                v-model="scope.row.PATRS_CODE"
                :fetch-suggestions="querySearchAsync"
                @select="((item)=>{handleSelect(item, index)})"
                placeholder="请输入备件分类码"
                :debounce=10
              ></el-autocomplete>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="CREATOR" label="维护人" align="center">
            <template slot-scope="scope">
              <el-input v-model="scope.row.CREATOR"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="CREATE_DATE" label="维护时间" align="center" sortable>
  ...
  export default {
    ...
    methods: {
      ...
      querySearchAsync(queryString, callback) {
        ...
        let _this=this;
        this.$httpGet('eqpapi/Q_EQP009',pars).then((res) => {
          for(let i of res['list']){
            i.value = i.PATRS_CODE; //将想要展示的数据作为value + '---' + i.PATRS_NAME
          }
          list = res['list'];
          callback(list);
        }).catch((error) => {
      ...
      handleSelect(item,index) {
        ...
        this.dataList.PATRS_CODE = item.PATRS_CODE
      },
      ...
      handleExp2XLSData() {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, '设备类型')
      })
      },
      ...
      SaveData: function () {
        let self = this
        self.$ajax.get('eqpapi/S_EQP037', {
            params:{
              EQP037DataS:JSON.stringify(self.dataList),
              DelEQP037S:JSON.stringify(self.deleteRow)
            }
          }
        ).then(
          function (res) {
      ...
      showDatas: function () {
        let self = this
        self.$ajax.get('eqpapi/Q_EQP037').then(
          function (response) {
